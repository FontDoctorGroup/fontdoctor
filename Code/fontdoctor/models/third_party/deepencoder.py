# =============================================================================
# Vendored from the official DeepSeek-OCR repository (MIT-style license).
# Source: https://github.com/deepseek-ai/DeepSeek-OCR
# File:   deepencoder.py  (HuggingFace: deepseek-ai/DeepSeek-OCR)
#
# DeepEncoder = SAM ViT-B (window attention) -> 16x conv compressor -> CLIP-L.
# FontDoctor keeps this module FROZEN and uses it as the OCR-specialized
# optical compression vision encoder E_ocr (paper Sec. 3.4).
# =============================================================================
...
# --- FontDoctor vendoring note -------------------------------------------------
# The two imports below are hard dependencies upstream but are (a) unused
# (`rearrange`) or (b) used once for an attribute-access dict (`adict`) in
# this file.  They are made optional so the vendored module only needs torch.
try:
    from einops import rearrange
except ImportError:  # pragma: no cover - rearrange is unused upstream
    rearrange = None

try:
    from easydict import EasyDict as adict
except ImportError:  # pragma: no cover - tiny fallback with attribute access
    class adict(dict):
